...
class Fetchers extends Component
{
    ...
    #[On('panels.sale.item-price.fetchers.assign-data')]
    public function assignData($id)
    {
        $this->authorize('sales_item_fetchers');

        $this->itemId = $id;
        $this->reset(['fetcher', 'link']);
        unset($this->fetchers);
        $this->js('$flux.modal(\'panels.sale.item-price.fetchers.modal\').show()');
    }
    ...
    {
        return view('livewire.panels.sale.item-price.fetchers');
    }
}
