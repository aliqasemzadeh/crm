...
new class extends Component
{
    ...
    #[On('panels.sale.party.addresses.assign-data')]
    public function assignData($PartyId): void
    {
        $this->authorize('sales_party_address');

        $this->party = Party::findOrFail($PartyId);
        Flux::modal('panels.sale.party.addresses.modal')->show();
    }
...
<flux:modal name="panels.sale.party.addresses.modal" class="md:w-1/3" flyout position="right">
    <div class="space-y-6">
        <div>
        ...
        <flux:table>
            ...
            <flux:table.rows>
                @foreach($this->addresses as $address)
                    <flux:table.row wire:key="sale-address-{{ $address->PartyAddressId ?? $loop->index }}">
