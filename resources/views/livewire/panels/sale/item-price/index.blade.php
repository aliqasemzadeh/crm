<div>
    ...
    <livewire:panels.accounting.price-note.invoices :grouping-id="$groupingId" :key="'invoices-'.$groupingId" />
    <livewire:panels.accounting.price-note.receipts :grouping-id="$groupingId" :key="'receipts-'.$groupingId" />
    ...
    <div class="mt-4" wire:key="price-note-content-{{ $groupingId }}">
        ...
        @if(\App\Models\Sepidar\INV\Item::where('CodingGroupRef', $groupingId)->count() > 0)
            <livewire:panels.accounting.price-note.items :grouping-id="$groupingId" :key="'items-'.$groupingId" />
        @else
            @php
