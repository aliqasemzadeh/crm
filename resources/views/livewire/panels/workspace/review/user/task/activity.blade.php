<flux:modal name="review-user-task-activity-modal" flyout position="right" class="md:w-1/3">
    <div class="space-y-6">
        <div>
        ...
        @if($task)
            <x-workspace.task.checklist
                ...
                add-modal-name="panels.workspace.review.user.task.activity.checklist.create.modal"
            />

            <flux:modal name="panels.workspace.review.user.task.activity.checklist.create.modal" flyout position="right" class="md:w-[420px]">
                <div class="space-y-5">
                    <div>
            ...
            <div class="space-y-3">
                <flux:checkbox.group :label="__('app.task.checklist')">
                    @foreach($task->checklists as $checklist)
                        <label wire:key="review-user-activity-checklist-{{ $checklist->id }}" class="flex items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 transition dark:border-zinc-700 {{ $checklist->is_done ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-white dark:bg-zinc-900/30' }}">
        ...
        @if($task)
            <form wire:submit="send" class="space-y-4">
                <flux:composer wire:model="body" placeholder="{{ __('app.task.activity.write_manager_note') }}">
