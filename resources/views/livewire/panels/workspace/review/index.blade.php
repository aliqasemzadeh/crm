        ...
        <flux:table.rows>
            @foreach ($this->tasks as $task)
                <flux:table.row :key="$task->id">
                    ...
                    <flux:table.cell>{{ $task->created_at->format('Y/m/d H:i') }}</flux:table.cell>
                    <flux:table.cell>{{ $task->done_at?->format('Y/m/d H:i') ?? '-' }}</flux:table.cell>
                    <flux:table.cell>
                    ...
                    <flux:table.cell>
                        <flux:button wire:click="openReviewModal({{ $task->id }})" variant="ghost" size="sm" icon="magnifying-glass">{{ __('app.task.review.review_button') }}</flux:button>
                    </flux:table.cell>
                </flux:table.row>
            @endforeach
        </flux:table.rows>
    ...
    <flux:modal name="review-task" variant="floating" class="md:w-lg">
        <form wire:submit="submitReview" class="space-y-6">
            ...
            @if($selectedTask)
                <div class="p-4 bg-zinc-50 dark:bg-zinc-800 rounded-lg space-y-2">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span>{{ $selectedTask->done_at?->format('Y/m/d H:i') ?? '-' }}</span>
                    </div>
                </div>
            @endif
            ...
            <div class="flex items-center justify-end gap-2">
                ...
                <flux:button type="submit" variant="primary">{{ __('app.task.review.submit_review') }}</flux:button>
            </div>
        </form>
    </flux:modal>
</div>
