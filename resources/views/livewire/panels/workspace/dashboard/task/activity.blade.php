<flux:modal name="panels.workspace.dashboard.task.activity.modal" flyout position="right" class="md:w-1/3">
    <div class="space-y-6">
        <div>
            ...
            <div class="space-y-3">
                <flux:checkbox.group :label="__('app.task.checklist')">
                    @foreach($task->checklists as $checklist)
                        <label wire:key="activity-checklist-{{ $checklist->id }}" class="flex items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 transition dark:border-zinc-700 {{ $checklist->is_done ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-white dark:bg-zinc-900/30' }}">
            ...
            <form wire:submit="send" class="space-y-4">
                <flux:composer wire:model="body" placeholder="{{ __('app.task.activity.write_report') }}">
                    <x-slot name="actionsLeading">
                        <input type="file" wire:model="files" multiple class="hidden" id="activity-files">
                        <flux:button size="sm" variant="subtle" icon="paper-clip" onclick="document.getElementById('activity-files').click()" />
                    </x-slot>
