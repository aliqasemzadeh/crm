...
new class extends Component
{
    ...
    public function save(): void
    {
        ...
        Cluster::query()->create($payload);

        $this->resetFormDefaults();
        $this->dispatch('panels.sale.item-cluster.index.render');
        Flux::modal('panels.sale.item-cluster.create.modal')->close();
...
<flux:modal name="panels.sale.item-cluster.create.modal" flyout position="right" class="md:w-[32rem]">
    <div class="space-y-6">
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('app.create_item_cluster') }}</flux:heading>
            ...
            <div class="space-y-2">
                ...
                >
                    ...
                    @foreach ($this->items as $item)
                        <flux:select.option value="{{ $item->ItemID }}" wire:key="cluster-create-item-{{ $item->ItemID }}">
            ...
            @if ($selectedItems !== [])
                <div class="space-y-2 max-h-64 overflow-y-auto">
                    @foreach ($selectedItems as $selected)
                        <div class="flex items-center justify-between gap-2 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-2" wire:key="cluster-create-selected-{{ $selected['id'] }}">
                            <div class="min-w-0">
