...
<flux:modal name="panels.service-center.repair.admission.modal" class="md:w-2/3" flyout position="right">
    <div class="space-y-6">
        <div>
        ...
        <form wire:submit="admission" method="post" class="space-y-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="space-y-4">
                    ...
                    <flux:field><flux:label>{{ __('app.device_type') }}</flux:label><flux:autocomplete wire:model="device_type" size="sm" type="text">@foreach($this->types as $type)<flux:autocomplete.item @click="$wire.fillDeviceType('{{ $type }}')">{{ $type }}</flux:autocomplete.item>@endforeach</flux:autocomplete><flux:error name="device_type" /></flux:field>
                    <flux:field><flux:label>{{ __('app.device_brand') }}</flux:label><flux:autocomplete wire:model="device_brand" size="sm" type="text">@foreach($this->brands as $brand)<flux:autocomplete.item @click="$wire.fillDeviceBrand('{{ $brand }}')">{{ $brand }}</flux:autocomplete.item>@endforeach</flux:autocomplete><flux:error name="device_brand" /></flux:field>
                    ...
                    <flux:field><flux:label>{{ __('app.warranty_type') }}</flux:label><flux:select wire:model="warranty_type" placeholder="{{ __('app.select_warranty_status') }}" size="sm"><flux:select.option>{{ __('app.select_warranty_status') }}</flux:select.option><flux:select.option value="no">{{ __('app.warranty_no') }}</flux:select.option><flux:select.option value="yes">{{ __('app.warranty_yes') }}</flux:select.option></flux:select><flux:error name="warranty_type" /></flux:field>
            ...
            <flux:button type="submit" class="w-full" variant="primary">{{ __('app.admission') }}</flux:button>
        </form>
    </div>
</flux:modal>