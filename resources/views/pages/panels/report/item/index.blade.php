...
new #[Layout('layouts.panels.report')] class extends Component
{
    ...
    {
        return app(ItemYearlySalesService::class)->topProductsBySales($this->selectedYear);
    }
};
?>
...
<div class="space-y-6">
    ...
    <flux:card wire:loading.class="opacity-60" wire:target="selectedYear,reload">
        <flux:heading size="lg" class="mb-4">{{ __('app.item_top_products_by_sales', ['year' => $selectedYear]) }}</flux:heading>
        ...
        <flux:table>
            ...
            <flux:table.rows>
                @forelse($this->topProductsBySales as $product)
                    <flux:table.row wire:key="item-top-product-sales-{{ $selectedYear }}-{{ $product['product_id'] }}">
                        <flux:table.cell class="tabular-nums">{{ $product['rank'] }}</flux:table.cell>
                        <flux:table.cell>{{ $product['name'] }}</flux:table.cell>
                        <flux:table.cell class="tabular-nums">
                            {{ number_format($product['average_price']) }}
